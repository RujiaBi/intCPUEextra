...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_IMATRIX(flag_t_index); // n_t x (n_f-1), -1 for fixed-zero cells, otherwise index into free flag_t vector
  ...
  const bool use_any_spde_1 = true;
  const bool use_any_spde_2 = true;
...
#if INTCPUE_ENABLE_Q_TIME
  matrix<Type> flag_t_centered_1(n_t, n_f - 1);
  ...
  if (use_q_diffs_time == 1) {
    for(int j=0; j<n_f-1; j++){
      for(int t=0; t<n_t; t++){
        ...
        if (idx >= 0) {
          flag_t_centered_1(t, j) = flag_t_1(idx);
          flag_t_centered_2(t, j) = flag_t_2(idx);
  ...
  for(int i=0; i<n_i; i++){	
    ...
    if (n_v > 0) ves_effect_1 = ves_v_1(vid);
	Type yq_effect_1 = yq_t_1(tid);
	Type flag_effect_1 = Type(0.0);
    if (n_f > 1 && fid > 0) flag_effect_1 = flag_f_1(fid-1);
	
    Type ves_effect_2 = Type(0.0);
    if (n_v > 0) ves_effect_2 = ves_v_2(vid);
    ...
    if (use_q_diffs_time == 1 && fid > 0) {
      ...
      flag_t_effect_1 = flag_t_centered_1(tid, j);
      flag_t_effect_2 = flag_t_centered_2(tid, j);
    }
#endif

	Type eta1 = ves_effect_1 + yq_effect_1 + flag_effect_1
#if INTCPUE_ENABLE_Q_TIME
      + flag_t_effect_1
#endif
#if INTCPUE_ENABLE_Q_SPATIAL
    ...
    Type eta2 = ves_effect_2 + yq_effect_2 + flag_effect_2
#if INTCPUE_ENABLE_Q_TIME
      + flag_t_effect_2
#endif
#if INTCPUE_ENABLE_Q_SPATIAL
