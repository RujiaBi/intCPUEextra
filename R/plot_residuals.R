...
#' \itemize{
#'   \item an observed-versus-predicted scatter plot
#'   \item a spatial plot of mean residuals aggregated within 2D bins
#' }
#'
#' For `intCPUE`, response-scale plots use the unconditional fitted mean, while
#' log-scale plots are restricted to positive observations and use the
#' conditional positive-CPUE mean.
#'
...
plot_residuals <- function(
  ...
  if (scatter_scale == "log") {
    ...
    plot_dat <- plot_dat[stats::complete.cases(plot_dat), , drop = FALSE]
    x_lab <- "Observed log positive CPUE"
    y_lab <- "Predicted log positive CPUE"
  } else {
    plot_dat <- data.frame(
  ...
  z_lab <- if (residual == "log") {
    "Mean log residual (positive observations only)"
  } else {
    "Mean raw residual (observed - predicted)"
  }

  spatial_dat <- data.frame(
    x = pred_df[[x_col]],
    y = pred_df[[y_col]],
    z = pred_df[[z_col]]
  )
  spatial_dat <- spatial_dat[stats::complete.cases(spatial_dat), , drop = FALSE]
